...
python -m hardware_hil.host.env_check \
  --output hardware_hil/results/pi5_environment_esp32c3_connected.json
```
...
```bash
python -m hardware_hil.host.env_check \
  --output hardware_hil/results/pi5_environment_arduino_r4_connected.json
```
...
You can use jumper wires and a breadboard for optional beginner LED tests, but not for the first CuKD-XAI HIL run. See `hardware_hil/docs/09_OPTIONAL_BREADBOARD_AND_JUMPER_GUIDE.md`.
