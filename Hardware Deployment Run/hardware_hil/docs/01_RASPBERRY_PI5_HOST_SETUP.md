...
python -m hardware_hil.host.env_check \
  --output hardware_hil/results/pi5_environment.json
```
...
```bash
python -m hardware_hil.host.env_check \
  --output hardware_hil/results/pi5_environment_after_board.json
```
...
For the HIL experiment, connect the ESP32-C3 or Arduino R4 to the Raspberry Pi 5 using the board USB cable only. Do not use Pi GPIO pins, UART pins, 5V pins, 3V3 pins, or GND jumpers. Follow `hardware_hil/docs/08_PHYSICAL_CONNECTIONS_AND_CABLES.md` before flashing or replaying vectors.
