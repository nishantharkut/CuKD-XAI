...
# Source: ../main/cukd_xai_colab.ipynb
...
# ----------------------------------------------------------------------------
SEEDS = [42, 123, 456, 789, 1001, 2024, 3141, 5678, 8192, 9999]        # 10 seeds for statistical validation
# SEEDS = [42]
...
PROTOCOL_ID = 'archive_random_split_train_scaler_controlled_tuning_v2'
GRID_TUNING_SEED = 42

SCRIPT_DIR = Path(__file__).resolve().parent
REPO_ROOT = SCRIPT_DIR.parents[2]
WSNDS_PATH = Path(os.environ.get(
    'WSNDS_PATH',
...
CICIOT_PATH = Path(os.environ.get(
    'CICIOT_PATH',
    str(REPO_ROOT / 'data' / 'ciciot2023' / 'CICIoT2023.csv'),
)).resolve()
...
#
# Historical v2.3 labels retained for artifact compatibility. These schedules
# do not establish compute matching: 3x33% + 3x66% + 24x100% is about 26.97
# full-dataset epoch equivalents, and global early stopping may terminate before
# the full-data stage. Do not interpret `_fair` as a causal fair-compute result.
#
...
# Scaling is intentionally deferred until after the raw split below. This is
# the sole methodological correction relative to the archived WSN-DS protocol.
scaler = None
...
def _iter_shuffled_batches(*tensors: torch.Tensor, batch_size: int):
    ...
    # DataLoader(shuffle=True, generator=None) consumes one global draw for its
    # iterator base seed and one for RandomSampler, then uses a local CPU
    # generator for randperm. Reproduce that sequence exactly so the execution
    # optimization does not change the archived shuffle stream.
...
# Random-forest fitting is deterministic for a fixed seed and split. The
# archived notebook redundantly fits identical RF objects in the Student A,
# Student B, and final-capture passes. These caches remove only that duplicate
# execution; every student route still trains independently.
_RF_RESULT_CACHE = {}
_RF_MODEL_CACHE = {}
_RF_SOFT_CACHE = {}
_RF_CALIBRATION_TIME_CACHE = {}
_RF_CALIBRATED_MODEL_CACHE = {}
...
def train_with_curriculum(model: nn.Module, X: torch.Tensor, y: torch.Tensor,
    ...
    #
    # Global early stopping uses the same patience value as train_standard but
    # does not make staged and full-data training compute matched. A stage can
    # stop before all examples are introduced.
    model = model.to(device)
...
def kd_grid_search(teacher: nn.Module, student_hidden: tuple,
    ...
    for T in T_grid:
        for a in alpha_grid:
            # Hold initialization and shuffle streams constant so validation
            # differences are attributable to T and alpha.
...
def run_all_configs(seed: int,
    ...
    rf_fit_reused = seed in _RF_RESULT_CACHE
    if not rf_fit_reused:
        ...
        _RF_RESULT_CACHE[seed] = {
            ...
            'fit_reused_within_rerun': False,
        }
        if seed == SEEDS[-1]:
            _RF_MODEL_CACHE[seed] = rf
    results['A_RF_500'] = copy.deepcopy(_RF_RESULT_CACHE[seed])
    results['A_RF_500']['fit_reused_within_rerun'] = rf_fit_reused
    if seed in _RF_MODEL_CACHE:
        models['A_RF_500'] = _RF_MODEL_CACHE[seed]
    ...
    # Historical v2.3 names retained in result keys.
    # C_fair   — nominal 3+3+24 staged schedule; not compute matched
    # C_ext    — CL with extended compute budget (+33% total epochs)
    # These are descriptive staged-training variants, not a causal compute ablation.

    # ----- Config C: historical `_fair` key; nominal staged schedule -----
    if verbose: print("[C_fair] CL-trained MLP (historical nominal schedule)...")
    t0 = time.perf_counter()
    ...
    # Alias C_CL_MLP_loss deterministically to the historical `_fair` variant
    ...
    # turning the aggregate into a chimera. This preserves the archived naming and
    # canonical-teacher choice; it does not establish compute matching.
    teacher_c = teacher_c_fair
    results['C_CL_MLP_loss'] = {**m_c_fair, '_source': 'historical _fair alias'}
    models['C_CL_MLP_loss'] = teacher_c

    # ----- Config C2: CL teacher with domain-based difficulty (nominal schedule) -----
    if verbose: print("[C2] CL-trained MLP (domain-based, historical nominal schedule)...")
    ...
    calibration_reused = seed in _RF_SOFT_CACHE
    if not calibration_reused:
        calibration_started = time.perf_counter()
        rf_calib = CalibratedClassifierCV(
            RandomForestClassifier(
                n_estimators=RF_CALIBRATION_CONFIG['n_estimators'],
                max_depth=RF_CALIBRATION_CONFIG['max_depth'],
                random_state=seed,
                n_jobs=-1,
            ),
            method=RF_CALIBRATION_CONFIG['calibration_method'],
            cv=RF_CALIBRATION_CONFIG['calibration_cv'],
        )
        rf_calib.fit(X_train, y_train)
        rf_soft = torch.tensor(
            rf_calib.predict_proba(X_train), dtype=torch.float32
        )
        _RF_SOFT_CACHE[seed] = rf_soft
        _RF_CALIBRATION_TIME_CACHE[seed] = time.perf_counter() - calibration_started
        if seed == SEEDS[-1]:
            _RF_CALIBRATED_MODEL_CACHE[seed] = rf_calib
        _save_rf_soft_cache(seed, rf_soft.numpy())
    else:
        rf_soft = _RF_SOFT_CACHE[seed]
    ...
    student_e_time = time.perf_counter() - student_started
    e_time = _RF_CALIBRATION_TIME_CACHE[seed] + student_e_time
    ...
    m_e['rf_calibration_time_sec'] = _RF_CALIBRATION_TIME_CACHE[seed]
    m_e['student_train_time_sec'] = student_e_time
    m_e['rf_soft_reused_within_rerun'] = calibration_reused
    results['E_KD_from_RF'] = m_e
    models['E_KD_from_RF'] = student_e
    if seed in _RF_CALIBRATED_MODEL_CACHE:
        models['E_RF_CALIBRATED_TEACHER'] = _RF_CALIBRATED_MODEL_CACHE[seed]
    if artifact_prefix:
        _save_model_artifact(
            f'{artifact_prefix}_seed_{seed}_E_KD_from_RF',
            student_e,
            seed=seed,
            student_hidden=student_hidden,
            config_name='E_KD_from_RF',
            kd_T=kd_T,
            kd_alpha=kd_alpha,
            metrics=m_e,
        )
    ...
    # Historical F_fair/F_ext keys distill from the two staged teachers. These
    # comparisons are descriptive because training compute is not matched.

    # ----- Config F_fair: historical key; KD from nominal-schedule CL teacher -----
    if verbose: print("[F_fair] KD from historical nominal-schedule CL-MLP...")
    ...
    # Alias F_KD_from_CL_MLP deterministically to the historical F_fair name.
    # Same reasoning as C_CL_MLP_loss above: a data-dependent alias breaks
    # multi-seed aggregation. This preserves the archived canonical selection; it
    # does not establish compute matching.
    results['F_KD_from_CL_MLP'] = {**m_f_fair, '_source': 'historical _fair alias'}
    models['F_KD_from_CL_MLP'] = student_f_fair

    # ----- Config G: KD from random-pacing MLP (same nominal staged schedule) -----
...
# First split (same seed-42 split as the archive, performed on raw features)
...
# Train-only scaling: this removes scaler-fit leakage, while the archived
# random-row split still permits exact feature duplicates across partitions.
...
# Quantify the protocol change without using the global scaler for any model input.
archived_global_scaler = StandardScaler().fit(X_all)
mean_shift_sd = np.abs(scaler.mean_ - archived_global_scaler.mean_) / np.where(
    archived_global_scaler.scale_ == 0, 1.0, archived_global_scaler.scale_
)
relative_scale_change = np.abs(scaler.scale_ / archived_global_scaler.scale_ - 1.0)
...
train_row_hashes = _row_hashes(X_train_raw, y_train_np)
validation_row_hashes = _row_hashes(X_val_raw, y_val_np)
test_row_hashes = _row_hashes(X_test_raw, y_test_np)
train_row_hash_set = set(map(int, train_row_hashes))
validation_row_hash_set = set(map(int, validation_row_hashes))
duplicate_audit = {
    'partition_policy': 'archived random-row split; duplicate groups are not constrained',
    'train_unique_row_hashes': int(len(train_row_hash_set)),
    'validation_unique_row_hashes': int(len(set(map(int, validation_row_hashes)))),
    'test_unique_row_hashes': int(len(set(map(int, test_row_hashes)))),
    'validation_rows_exactly_present_in_train': int(sum(
        int(value) in train_row_hash_set for value in validation_row_hashes
    )),
    'test_rows_exactly_present_in_train': int(sum(
        int(value) in train_row_hash_set for value in test_row_hashes
    )),
    'test_rows_exactly_present_in_validation': int(sum(
        int(value) in validation_row_hash_set for value in test_row_hashes
    )),
}

dataset_sha256 = _sha256_file(WSNDS_PATH)
split_hashes = {
...
preprocessing_report = {
    ...
    'protocol': 'train-only StandardScaler under the archived seed-42 stratified random-row split',
    'claim_boundary': 'Corrects scaler fitting only; exact duplicate records may cross archived random-row partitions.',
    'dataset_path': str(WSNDS_PATH),
    'dataset_sha256': dataset_sha256,
    'split_random_state': 42,
    'split_shapes': {
        'train': list(X_train_np.shape),
        'validation': list(X_val_np.shape),
        'test': list(X_test_np.shape),
    },
    'feature_names': FEATURE_NAMES,
    'class_names': CLASS_NAMES,
    'scaler_fit_partition': 'train',
    'model_input_scaler': 'train_partition_only',
    'global_scaler_role': 'diagnostic_only_never_transforms_model_inputs',
    'scaler_mean': scaler.mean_.tolist(),
    'scaler_scale': scaler.scale_.tolist(),
    'scaler_sha256': scaler_sha256,
    'split_hashes': split_hashes,
    'duplicate_audit': duplicate_audit,
    'archived_global_scaler_diagnostic': {
        'mean_shift_in_global_sd_max': float(mean_shift_sd.max()),
        'mean_shift_in_global_sd_mean': float(mean_shift_sd.mean()),
        'relative_scale_change_max': float(relative_scale_change.max()),
        'relative_scale_change_mean': float(relative_scale_change.mean()),
    },
    'seeds': SEEDS,
    'environment': {
...
_atomic_write_json(OUTPUT_DIR / 'leakage_free_preprocessing.json', preprocessing_report)
np.savez(
    OUTPUT_DIR / 'train_fitted_scaler.npz',
    mean=scaler.mean_,
    scale=scaler.scale_,
    var=scaler.var_,
...
def _save_rf_soft_cache(seed: int, probabilities: np.ndarray) -> None:
    array = np.asarray(probabilities, dtype=np.float32)
    if array.shape != (len(X_train_np), NUM_CLASSES):
        raise RuntimeError(f'Unexpected RF probability shape for seed {seed}: {array.shape}')
    cache_path = OUTPUT_DIR / f'rf_soft_seed_{seed}.npy'
    temporary = cache_path.with_suffix('.npy.tmp')
    with temporary.open('wb') as handle:
        np.save(handle, array, allow_pickle=False)
    temporary.replace(cache_path)
    _atomic_write_json(OUTPUT_DIR / f'rf_soft_seed_{seed}.manifest.json', {
        'protocol_id': PROTOCOL_ID,
        'seed': int(seed),
        'dataset_sha256': dataset_sha256,
        ...
        'probabilities_sha256': _sha256_arrays(array),
...
def _save_model_artifact(name: str, model: nn.Module, *, seed: int,
                         student_hidden: tuple, config_name: str,
                         kd_T: float, kd_alpha: float,
                         metrics: dict) -> None:
    artifact_path = MODEL_ARTIFACT_DIR / f'{name}.pt'
    ...
    torch.save({
        ...
        'split_hashes': split_hashes,
        'scaler_sha256': scaler_sha256,
        'kd_hyperparameters': {'T': float(kd_T), 'alpha': float(kd_alpha)},
        'metrics': metrics,
        'state_dict': cpu_state,
    }, temporary)
    temporary.replace(artifact_path)
    _atomic_write_json(artifact_path.with_suffix('.manifest.json'), {
...
print('Scaler fit partition: train only')


def write_seed_checkpoint(student: str, seed: int, payload: dict) -> None:
    path = OUTPUT_DIR / f'checkpoint_student_{student}_seed_{seed}.json'
...
grid_result = {
    'mode': 'quick_default_no_search',
    'tuning_seed': GRID_TUNING_SEED,
    'best': {'T': BEST_T, 'alpha': BEST_ALPHA, 'val_f1': None},
    'all': [],
}
if not QUICK_MODE:
    ...
    grid_result['mode'] = 'controlled_single_seed_grid'
    grid_result['tuning_seed'] = GRID_TUNING_SEED
    ...
    _atomic_write_json(OUTPUT_DIR / 'kd_grid_search.json', grid_result)
...
for seed in SEEDS:
    run_payload = run_all_configs(
        ...
        artifact_prefix='student_A',
        return_models=(seed == SEEDS[-1]),
        verbose=True,
    )
    if seed == SEEDS[-1]:
        all_seed_results_A[seed], final_models = run_payload
    else:
        all_seed_results_A[seed] = run_payload
    write_seed_checkpoint('A', seed, all_seed_results_A[seed])

# Preserve the exact final-seed neural models used for SHAP, quantization, and
# latency analysis. RF-KD students are already saved for every seed above.
for _config_name, _model in final_models.items():
    if isinstance(_model, StudentMLP) and _config_name in all_seed_results_A[SEEDS[-1]]:
        _save_model_artifact(
            f'student_A_seed_{SEEDS[-1]}_analysis_{_config_name}',
            _model,
            seed=SEEDS[-1],
            student_hidden=STUDENT_A_HIDDEN,
            ...
            kd_T=BEST_T,
            kd_alpha=BEST_ALPHA,
            metrics=all_seed_results_A[SEEDS[-1]][_config_name],
        )

if 'A_RF_500' in final_models:
    _rf_artifact = MODEL_ARTIFACT_DIR / f'rf_reference_seed_{SEEDS[-1]}.pkl'
    _rf_temporary = _rf_artifact.with_suffix('.pkl.tmp')
    with _rf_temporary.open('wb') as _handle:
        pickle.dump(final_models['A_RF_500'], _handle, protocol=pickle.HIGHEST_PROTOCOL)
    _rf_temporary.replace(_rf_artifact)
    _atomic_write_json(_rf_artifact.with_suffix('.manifest.json'), {
        'artifact_file': _rf_artifact.name,
        'artifact_sha256': _sha256_file(_rf_artifact),
        'protocol_id': PROTOCOL_ID,
        'config_name': 'A_RF_500_reference_for_cross_model_shap',
        'seed': int(SEEDS[-1]),
        'dataset_sha256': dataset_sha256,
        'scaler_sha256': scaler_sha256,
        'script_sha256': script_sha256,
...
if not QUICK_MODE:
    ...
    for seed in SEEDS:
        all_seed_results_B[seed] = run_all_configs(
            ...
            kd_T=BEST_T, kd_alpha=BEST_ALPHA,
            artifact_prefix='student_B',
            verbose=False,
        )
        write_seed_checkpoint('B', seed, all_seed_results_B[seed])


REQUIRED_CONFIGS = {
    'A_RF_500', 'B_Full_MLP', 'C_CL_MLP_loss_fair',
    'C_CL_MLP_loss_ext', 'C2_CL_MLP_domain', 'D_Small_MLP',
    'E_KD_from_RF', 'E2_KD_from_MLP', 'F_KD_from_CL_MLP_fair',
    'F_KD_from_CL_MLP_ext', 'G_KD_random_pacing',
    'I_KD_from_SMOTE_MLP',
}


def _validate_complete_results(student: str, seed_results: dict) -> None:
    if list(seed_results.keys()) != SEEDS:
        raise RuntimeError(
            f'{student} seed coverage mismatch: {list(seed_results.keys())} != {SEEDS}'
        )
    for seed in SEEDS:
        missing = REQUIRED_CONFIGS - set(seed_results[seed])
        if missing:
            raise RuntimeError(f'{student} seed {seed} missing configs: {sorted(missing)}')
        for config_name in REQUIRED_CONFIGS:
            metrics = seed_results[seed][config_name]
            for metric_name in ('accuracy', 'macro_f1'):
                value = float(metrics[metric_name])
                if not np.isfinite(value):
                    raise RuntimeError(
                        f'{student} seed {seed} {config_name} has non-finite {metric_name}'
                    )


_validate_complete_results('Student A', all_seed_results_A)
if not QUICK_MODE:
    _validate_complete_results('Student B', all_seed_results_B)
_atomic_write_json(OUTPUT_DIR / 'completion_gate.json', {
    'status': 'passed',
    'protocol_id': PROTOCOL_ID,
    'seeds': SEEDS,
    'required_configs': sorted(REQUIRED_CONFIGS),
    'student_A_complete': True,
    'student_B_complete': not QUICK_MODE,
})

# The last Student A pass retained its models for SHAP and benchmarks. The
# archived notebook reran that deterministic seed solely to recover objects.
final_results = all_seed_results_A[SEEDS[-1]]
if final_models is None:
    raise RuntimeError('Final-seed models were not retained.')
print("\n>>> Reusing retained final-seed Student A models for SHAP/benchmarks.")
...
def wilcoxon_test(seed_results: dict, cfg_a: str, cfg_b: str,
    ...
    if len(vals_a) != len(vals_b) or len(vals_a) < 2:
        return {'stat': None, 'p': None, 'diff_mean': None,
                'verdict': 'insufficient data'}
    diffs = np.array(vals_a) - np.array(vals_b)
    if np.all(diffs == 0):
        return {'stat': 0.0, 'p': 1.0, 'diff_mean': 0.0,
                'verdict': 'identical'}
    try:
...
key_comparisons = [
    ...
    ('C_CL_MLP_loss_fair', 'B_Full_MLP', "Does CL help under the historical nominal schedule? (C_fair vs B)"),
    ('C_CL_MLP_loss_ext',  'B_Full_MLP', "Does CL help teacher at EXT budget? (C_ext vs B)"),
    # Student-level CL question (the core claim)
    ('F_KD_from_CL_MLP_fair', 'E2_KD_from_MLP', "Does CL cascade under the historical nominal schedule? (F_fair vs E2)"),
...
for a, b, desc in key_comparisons:
    ...
    diff_mean = w.get('diff_mean')
    diff_str = 'n/a' if diff_mean is None else f"{diff_mean:+.4f}"
    print(f"{desc}")
    print(f"  {a}: {a_mean:.4f}")
    print(f"  {b}: {b_mean:.4f}")
    print(f"  diff: {diff_str}  |  p={w['p']}  |  {w['verdict']}\n")
...
try:
    ...
    # This is a cross-model comparison: Config F was trained from a CL-MLP,
    # while Config A is an independent RF baseline. It is not teacher-student
    ...
    rf_reference_for_shap = final_models['A_RF_500']
    ...
    # ---- Independent RF reference: TreeExplainer ----
    print("\nComputing SHAP for independent RF reference (TreeExplainer)...")
    rf_explainer = shap.TreeExplainer(rf_reference_for_shap)
    ...
    rf_reference_imp_df = pd.DataFrame({
        'feature': FEATURE_NAMES,
        'rf_reference_shap': rf_global,
    }).sort_values('rf_reference_shap', ascending=False)

    print("\nIndependent RF reference top-10 features (global):")
    print(rf_reference_imp_df.head(10).to_string(index=False))
    ...
    # Rank correlation between two independently trained model routes.
    ...
    print("Interpretation: cross-model feature-ranking similarity only; not teacher-student transfer")
    ...
    shap_results = {
        'comparison_type': 'cross_model_feature_ranking',
        'student_config': 'F_KD_from_CL_MLP',
        'reference_config': 'A_RF_500',
        'actual_teacher_student_pair': False,
        'claim_boundary': 'Does not measure attribution transfer from Config F actual CL-MLP teacher.',
        'student_global_importance': student_imp_df.to_dict('records'),
        'rf_reference_global_importance': rf_reference_imp_df.to_dict('records'),
        # Compatibility alias for archived consumers; scope is defined above.
        'teacher_global_importance': pd.DataFrame({
            'feature': FEATURE_NAMES,
            'teacher_shap': rf_global,
        }).sort_values('teacher_shap', ascending=False).to_dict('records'),
...
# Plots all three schedules descriptively; this is not a compute-matched comparison.
curve_configs = [
    ('B_Full_MLP', 'B (no CL)', 'C0'),
    ('C_CL_MLP_loss_fair', 'C_fair (CL, nominal schedule)', 'C1'),
...
if RUN_CICIOT and CICIOT_PATH.is_file():
    print("\n" + "=" * 60)
    ...
    try:
        ...
        X_ctrval_raw, X_cte_raw, y_ctrval, y_cte = train_test_split(
            X_c, y_c, test_size=0.15, random_state=42, stratify=y_c
        )
        X_ctr_raw, X_cv_raw, y_ctr, y_cv = train_test_split(
            X_ctrval_raw, y_ctrval, test_size=0.1765,
            random_state=42, stratify=y_ctrval
        )
        scaler_c = StandardScaler()
        X_ctr = scaler_c.fit_transform(X_ctr_raw).astype(np.float32, copy=False)
        X_cv = scaler_c.transform(X_cv_raw).astype(np.float32, copy=False)
        X_cte = scaler_c.transform(X_cte_raw).astype(np.float32, copy=False)
...
agg_B = (
    aggregate_multi_seed(all_seed_results_B)
    if len(all_seed_results_B) > 0
    else pd.DataFrame()
)
...
final_output = {
    ...
    'aggregate_student_B': agg_B.to_dict('records'),
    'kd_hyperparameters': {'T': BEST_T, 'alpha': BEST_ALPHA},
    'kd_grid_search': grid_result,
    ...
    'class_names': CLASS_NAMES,
    'feature_names': FEATURE_NAMES,
    'preprocessing_protocol': preprocessing_report,
    'duplicate_audit': duplicate_audit,
    'completion_gate': {
        'student_A_complete': True,
        'student_B_complete': not QUICK_MODE,
    ...
    'result_scope': (
        'Train-only StandardScaler under the archived seed-42 stratified '
        'random-row split. Exact duplicate records may cross partitions; '
        'this run does not evaluate a duplicate-group-constrained split. '
        'Ten optimizer seeds share one fixed split. Historical `_fair` curriculum '
        'labels are not compute-matched, and the SHAP comparison is cross-model '
        'rather than RF-to-RF-KD attribution transfer.'
    ),
}

_atomic_write_json(OUTPUT_DIR / 'cukd_xai_results.json', final_output)
print("\nSaved cukd_xai_results.json")

# Save aggregate CSV for the paper
agg_A.to_csv(OUTPUT_DIR / 'wsnds_results_student_A.csv', index=False)
print("Saved wsnds_results_student_A.csv")
if not agg_B.empty:
    agg_B.to_csv(OUTPUT_DIR / 'wsnds_results_student_B.csv', index=False)
