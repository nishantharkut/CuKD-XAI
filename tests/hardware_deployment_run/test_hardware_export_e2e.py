...
exp = load_module("wsnds_rfkd_hil_export", EXPORT_ROOT / "export_wsnds_student_a_rfkd_int8.py")
run_e2e = load_module("wsnds_rfkd_hil_e2e", EXPORT_ROOT / "run_wsnds_student_a_rfkd_e2e.py")
...
class HardwareExportE2ETests(unittest.TestCase):
    ...
    def test_write_header_supports_student_b_rfkd_dimensions(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = exp.write_header(
                output_path,
                quantized_layers,
                "results/runtime/onnx_openvino/wsnds/tmp/E_student_B_KD_from_RF_fp32.pt",
            )
    ...
    def test_integer_preprocessing_c_core_compiles_with_generated_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source = EXPORT_ROOT / "wsnds_preprocess_int16.c"
    ...
    def test_generate_e2e_artifacts_embeds_raw_preprocess_vectors(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = exp.generate_e2e_artifacts(
                ...
                dataset=dataset,
                dataset_csv=Path("data/wsnds/WSN-DS.csv"),
    ...
    def test_e2e_runner_passes_model_label_to_exporter_command(self):
        cmd = run_e2e.build_export_command(
            ...
            state_dict="results/runtime/onnx_openvino/wsnds/tmp/E_student_B_KD_from_RF_fp32.pt",
            output_dir=Path("deployment/firmware_export/wsnds_rfkd_hil/generated_student_b_rfkd_hil_full"),
            dataset_csv="data/wsnds/WSN-DS.csv",
