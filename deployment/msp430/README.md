...
```bash
python3 deployment/msp430/run_wsnds_student_a_rfkd_e2e.py \
  --state-dict results/runtime/onnx_openvino/wsnds/tmp/E_student_A_KD_from_RF_fp32.pt \
  --dataset-csv data/wsnds/WSN-DS.csv \
  --output-dir deployment/msp430/generated_student_a_rfkd_e2e \
...
The default output directory is `deployment/msp430/generated_student_a_rfkd_e2e/`. The final full-test output used for reporting is `deployment/msp430/generated_student_a_rfkd_e2e_v2/`.
...
From the full 56,200-vector WSN-DS software export already recorded in `deployment/msp430/generated_student_a_rfkd_e2e_v2/equivalence_report.json`:
...
See `deployment/msp430/MSP430_CROSS_COMPILE_REPORT.md` for the full MSP430 report.

## WSN Mote Next Step

Use `deployment/msp430/WSN_MOTE_HARDWARE_REQUEST.md` when asking for lab hardware. The immediate experiment should be compile-size plus serial self-test on an actual WSN-class board, not Raspberry Pi benchmarking.
...
gcc -std=c99 -Wall -Wextra -Os \
  -Ideployment/msp430/generated_student_a_rfkd_e2e_v2 \
  ...
  -o deployment/msp430/generated_student_a_rfkd_e2e_v2/cukd_student_a_rfkd_self_test

deployment/msp430/generated_student_a_rfkd_e2e_v2/cukd_student_a_rfkd_self_test
```
...
msp430-elf-gcc -mmcu=msp430f1611 -std=c99 -Wall -Wextra -Os \
  -Ideployment/msp430/generated_student_a_rfkd_e2e_v2 \
  -c deployment/msp430/wsnds_preprocess_int16.c \
  -o deployment/msp430/msp430_build_v2/wsnds_preprocess_int16_msp430.o

msp430-elf-gcc -mmcu=msp430f1611 -std=c99 -Wall -Wextra -Os \
  -Ideployment/msp430/generated_student_a_rfkd_e2e_v2 \
  -c deployment/msp430/wsnds_student_a_rfkd_int8_inference.c \
  -o deployment/msp430/msp430_build_v2/wsnds_student_a_rfkd_int8_inference_msp430.o

msp430-elf-size -A deployment/msp430/msp430_build_v2/wsnds_preprocess_int16_msp430.o
msp430-elf-size -A deployment/msp430/msp430_build_v2/wsnds_student_a_rfkd_int8_inference_msp430.o
```
...
- `deployment/msp430/msp430_build_v2/`: MSP430F1611 cross-compiled smoke firmware, object files, and disassembly evidence.
