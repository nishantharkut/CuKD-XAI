"""Generate the final four-pair FG-DS HIL report from passed evidence only."""
...
try:
    ...
    from .record_fgds_compile_evidence import (
        BOARD_FQBN_PREFIXES,
        FLASH_PATTERN,
        RAM_PATTERN,
        parsed_match,
    ...
    from .verify_results_fgds_strict import read_mcu, read_reference
except ImportError:
    from hil_common import compute_classification_metrics, summarize_latency
    from record_fgds_compile_evidence import (
        BOARD_FQBN_PREFIXES,
...
HIL_PROTOCOL = "fgds_seed42_strict_hil_three_stage_v1"
FULL_TEST_ROWS = 56301
CALIBRATION_ROWS = 262197
MAXIMUM_MACRO_F1_DROP = 0.03
REQUIRED_STAGES = {
    "smoke_10": 10,
    "validation_1000": 1000,
    "full_56301": FULL_TEST_ROWS,
}
...
def validate_compile_identity(
    ...
    if compile_path is not None:
        ...
        if (
            ...
            or float(macro_f1_drop) > MAXIMUM_MACRO_F1_DROP
        ):
        ...
        if len(references) != FULL_TEST_ROWS or set(references) != set(
            range(FULL_TEST_ROWS)
        ):
            raise RuntimeError(f"Copied strict reference row contract differs: {label}")
        ordered = [references[index] for index in range(FULL_TEST_ROWS)]
        source_row_indices = [item["source_row_index"] for item in ordered]
        if min(source_row_indices) < 0 or len(set(source_row_indices)) != FULL_TEST_ROWS:
            raise RuntimeError(
        ...
        if calibration_saturation.get("rows_audited") != CALIBRATION_ROWS:
...
def validate_hil_source_evidence(
    ...
    run_script = Path(__file__).resolve().parents[1] / "scripts" / "run_fgds_hil.sh"
    ...
    if len(references) != FULL_TEST_ROWS or set(references) != set(
        ...
        raise RuntimeError(
            f"Portable FG-DS reference is not rows 0..{FULL_TEST_ROWS - 1}: {label}"
        )
    source_row_indices = [
        references[index]["source_row_index"] for index in range(FULL_TEST_ROWS)
    ]
    if min(source_row_indices) < 0 or len(set(source_row_indices)) != FULL_TEST_ROWS:
        raise RuntimeError(f"Portable HIL reference source rows are invalid: {label}")
    implementation_hashes = {
        "stream_script_sha256": sha256_file(Path(__file__).with_name("stream_vectors_fgds_strict.py")),
        ...
        "verification_script_sha256": sha256_file(Path(__file__).with_name("verify_results_fgds_strict.py")),
    ...
    if stage != "full_56301":
        raise RuntimeError(f"Final HIL input must be the full_56301 metrics stage: {label}")
    ...
    if (
        sequence.get("expected") != FULL_TEST_ROWS
        or sequence.get("completed") != FULL_TEST_ROWS
    ):
        raise RuntimeError(f"HIL sequence is not the full 56,301 rows: {label}")
    ...
    if sequence.get("status_counts") != {"OK": FULL_TEST_ROWS}:
    ...
    rows = read_mcu(mcu_path)
    if [row["row_id"] for row in rows] != list(range(FULL_TEST_ROWS)):
...
def main() -> int:
    ...
    for label in sorted(EXPECTED_LABELS):
        ...
        if evidence.get("completed_vectors") != FULL_TEST_ROWS:
    ...
    if len(per_student_vector_hashes["student_A"] | per_student_vector_hashes["student_B"]) != 1:
        raise RuntimeError("Student A and Student B do not use byte-identical replay vectors")
    ...
    json_payload = {
        "status": "passed",
        "run_count": 4,
        "full_test_board_predictions": 4 * FULL_TEST_ROWS,
        ...
        "claim_boundary": (
            ...
            "TelosB execution. The replay uses the preserved seed-42 RF-KD models "
            "and the 56,301-row feature-group-disjoint test partition. It establishes "
            "execution fidelity for that seed, not five-seed hardware replication."
        ),
