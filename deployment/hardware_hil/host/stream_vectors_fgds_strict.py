"""Replay FG-DS vectors after verifying export, bundle, and device identity."""
...
STRICT_EXPORT_PROTOCOL = "wsnds_feature_group_split_train_only_scaler_5seed_v1"
...
def verify_export_report(generated_dir: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if macro_f1_drop > MAXIMUM_MACRO_F1_DROP:
        raise RuntimeError("FG-DS export macro-F1 drop exceeds the frozen 0.03 tolerance")
    if gates.get("passes_frozen_0_03_reporting_tolerance") is not True:
        raise RuntimeError("FG-DS export does not record a passed 0.03 reporting tolerance")
    if gates.get("passes_original_0_01_code_level_gate") != (macro_f1_drop <= 0.01):
        raise RuntimeError("FG-DS export records an inconsistent original 0.01 gate result")
    return report
...
def verify_bundle(bundle_dir: Path, export_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    bundler = Path(__file__).with_name("prepare_fgds_firmware_bundle.py")
