"""Fail-closed verification of sequence, identity, predictions, logits, and latency."""
...
try:
    ...
    from .stream_vectors_strict import verify_bundle, verify_export
except ImportError:
    from hil_common import compute_classification_metrics, summarize_latency
    from stream_vectors_strict import verify_bundle, verify_export
...
def main() -> int:
    ...
    if not 1 <= args.expected_count <= 56200:
        raise ValueError("--expected-count must be between 1 and 56200")
    ...
    if len(references) != 56200 or set(references) != set(range(56200)):
        raise RuntimeError("Strict reference must contain row IDs 0..56199 exactly once")
    replay_source_rows = read_replay_source_rows(
        generated_dir / "hil_replay_vectors.csv"
    )
    if len(replay_source_rows) != 56200 or set(replay_source_rows) != set(range(56200)):
        raise RuntimeError("Strict replay must contain row IDs 0..56199 exactly once")
    source_values = list(replay_source_rows.values())
    if min(source_values) < 0 or len(set(source_values)) != 56200:
