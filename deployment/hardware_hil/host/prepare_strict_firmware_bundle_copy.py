"""COPY of prepare_strict_firmware_bundle.py (original untouched).

Imports stream_vectors_strict_copy for the 0.03 macro-F1 drop gate.
"""
...
try:
    from .stream_vectors_strict_copy import verify_export
except ImportError:
    from stream_vectors_strict_copy import verify_export
...
def main() -> int:
    ...
    template_text = source_snapshots[template.name].decode("utf-8").replace("\r\n", "\n")
    ...
    try:
        ...
        (staging / sketch_name).write_text(transformed, encoding="utf-8")
        ...
        manifest = {
            ...
            "files": files,
            "claim_boundary": (
                "USB serial replay of extracted WSN-DS 17-feature records; "
