...
preserve_failed_run() {
  ...
  if [[ -d "${RESULT_DIR}" ]]; then
    ...
    echo "strict HIL failed; partial evidence preserved at: ${failed_dir}" >&2
  fi
  exit "${status}"
}
...
run_stage() {
  ...
  local limit_args=()
  if [[ "${count}" -ne 56200 ]]; then
    limit_args=(--limit "${count}")
  fi

  timeout --foreground --signal=TERM --kill-after=30s "${wall_timeout}" \
    python deployment/hardware_hil/host/stream_vectors_strict.py \
    --port "${PORT}" \
  ...
  timeout --foreground --signal=TERM --kill-after=30s 30m \
    python deployment/hardware_hil/host/verify_results_strict.py \
...
run_stage full_56200 56200 5.0 6h
...
expected_stage_counts = {"smoke_10": 10, "validation_1000": 1000, "full_56200": 56200}
expected_names = {
    f"{stage}_{suffix}"
...
temporary.write_text(json.dumps({
    "status": "complete",
    "protocol_id": "strict_hil_three_stage_v1",
...
trap - ERR INT TERM HUP
echo "strict HIL completed: ${FINAL_RESULT_DIR}"
