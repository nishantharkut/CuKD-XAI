...
# This generated copy is intentionally edited only in the leakage-free workspace.
...
OUTPUT_DIR = Path(os.environ.get(
    'CUKD_OUTPUT_DIR',
    str(REPO_ROOT / 'results' / 'wsnds' / 'leakage_free_rerun' / 'main_10seed'),
)).resolve()
...
def run_all_configs(seed: int,
    ...
    if not calibration_reused:
        ...
        np.save(OUTPUT_DIR / f'rf_soft_seed_{seed}.npy', rf_soft.numpy())
    else:
...
# Leakage-free preprocessing: learn all scaler statistics from training only.
...
preprocessing_report = {
    'protocol': 'train-only StandardScaler after seed-42 stratified raw split',
...
with open('leakage_free_preprocessing.json', 'w', encoding='utf-8') as handle:
        ...
        json.dump(
            {
                'student': student,
                'seed': int(seed),
                'preprocessing': 'train-only scaler',
                'results': payload,
            },
            handle,
            indent=2,
...
final_output = {
    ...
    'result_scope': 'leakage-free WSN-DS rerun; archived files are unchanged',
}
