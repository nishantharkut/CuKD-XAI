"""Run the ten-seed, feature-group-disjoint WSN-DS confirmation.

This additive runner preserves the original five-seed confirmation and extends
the same training logic to the complete publication seed set in a new output
directory. The default mode is preflight and performs no model training.
Training modes require --confirm-training.
"""
...
GROUP_PROTOCOL = "wsnds_feature_group_split_train_only_scaler_10seed_v2"
...
def aggregate_results(
    ...
    atomic_write_json(path, {
        ...
        "inference_boundary": (
            "Ten-seed exact-feature-group confirmation summaries quantify "
            "optimizer-seed variation on one fixed group-disjoint split; "
            "they are not a matched causal ablation against the archived random-row "
            "route and do not estimate variation across independently sampled "
            "train/test splits."
            if protocol_id == GROUP_PROTOCOL
            else
...
def run_training_mode(
    ...
    if mode == "deployment":
        ...
        root = output_root / "feature_group_10seed"
