...
static void send_status(uint32_t row_id, cukd_status_t status) {
...
static uint8_t argmax_logits(const int16_t logits[CUKD_OUTPUT_DIM]) {
    uint8_t best = 0;
...
static void process_line(const char *line) {
    ...
    preprocess_end = now_us();
    cukd_forward_q15(input_q15, logits);
    pred = argmax_logits(logits);
#ifdef CUKD_HIL_VERIFY_PREDICT_WRAPPER
    if (cukd_predict_q15(input_q15) != pred) {
        send_status(request.row_id, CUKD_STATUS_INTERNAL_ERROR);
        return;
    }
#endif
    inference_end = now_us();
    ...
    while (Serial.available() > 0) {
        ...
        if (ch == '\n') {
            ...
            if (rx_len + 1 < sizeof(rx_line)) {
                ...
                rx_len = 0;
                send_status(0, CUKD_STATUS_BAD_LENGTH);
            }
        }
    }
}
