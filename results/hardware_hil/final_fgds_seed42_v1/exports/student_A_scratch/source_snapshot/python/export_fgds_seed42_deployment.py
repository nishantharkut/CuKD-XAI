"""Export a provenance-locked FG-DS seed-42 RF-KD deployment model.

This additive exporter consumes the completed five-seed feature-group-disjoint
confirmation artifacts. It selects the already-preserved seed-42 RF-KD state;
it does not retrain a model, refit a teacher, choose a seed, or alter the split.
"""
...
EXPECTED_SEEDS = [42, 123, 456, 789, 1001]
MINIMUM_FIXED_FP32_AGREEMENT = 0.99
MAXIMUM_MACRO_F1_DROP = 0.03
ORIGINAL_CODE_LEVEL_MACRO_F1_DROP = 0.01
...
def verify_execution_contract(
    ...
    if execution.get("bound_teacher_soft_target_provenance") is not None:
        raise RuntimeError("FG-DS execution unexpectedly binds one shared RF target cache")
    teacher_provenance = completion.get("teacher_soft_target_provenance")
    if not isinstance(teacher_provenance, dict):
        raise RuntimeError("FG-DS seed completion lacks RF soft-target provenance")
    ...
    calibration = teacher_provenance.get("calibration_audit")
    if (
        teacher_provenance.get("source_type") != "fresh_calibrated_rf_soft_targets"
        ...
        raise RuntimeError("FG-DS seed-42 RF calibration provenance is invalid")
    content_sha256 = teacher_provenance.get("train_probability_content_sha256")
    if not isinstance(content_sha256, str) or len(content_sha256) != 64:
        raise RuntimeError("FG-DS seed-42 RF probability content hash is invalid")
...
def load_verified_context(
    ...
    computed_indices_hash = sha256_arrays(
        split["train_indices"],
        split["validation_indices"],
        split["test_indices"],
    )
    if computed_indices_hash != preprocessing.get("split_indices_sha256"):
        raise RuntimeError("Recomputed FG-DS split-index hash differs from its contract")
    computed_transformed_hashes = {
        name: sha256_arrays(scaled[f"X_{name}"])
        for name in ["train", "validation", "test"]
    }
    if computed_transformed_hashes != preprocessing.get("transformed_split_hashes"):
        raise RuntimeError("Recomputed FG-DS transformed hashes differ from their contract")
    expected_sizes = {"train": 262197, "validation": 56163, "test": 56301}
    observed_sizes = {
        name: int(len(split[f"{name}_indices"]))
        for name in expected_sizes
    }
    if observed_sizes != expected_sizes or preprocessing.get("split_sizes") != expected_sizes:
        raise RuntimeError(f"FG-DS split sizes differ: {observed_sizes!r}")
    overlap_audit = split["group_audit"]
    if any(
        overlap_audit.get(key) != 0
        for key in [
        ...
        raise RuntimeError("FG-DS split does not have zero cross-partition feature overlap")
    ...
    teacher_provenance = completion["teacher_soft_target_provenance"]
    probability_item = next(
        (
            item for item in seed_manifest.get("files", [])
            if item.get("path") == "rf_train_probabilities.npy"
        ),
        None,
    )
    probability_path = seed_root / "rf_train_probabilities.npy"
    if probability_item is None or not probability_path.is_file():
        raise RuntimeError("FG-DS seed manifest does not bind RF train probabilities")
    train_probabilities = np.load(probability_path, allow_pickle=False)
    if train_probabilities.shape != (expected_sizes["train"], len(CLASS_NAMES)):
        raise RuntimeError("FG-DS seed-42 RF train probabilities have the wrong shape")
    if not np.isfinite(train_probabilities).all():
        raise RuntimeError("FG-DS seed-42 RF train probabilities are non-finite")
    if sha256_arrays(train_probabilities) != teacher_provenance[
        "train_probability_content_sha256"
    ]:
        raise RuntimeError("FG-DS seed-42 RF probability content hash differs")
    ...
    return {
        ...
        "teacher_probability_path": probability_path,
        "teacher_soft_target_provenance": teacher_provenance,
    }
...
def normalize_generated_headers(output_dir: Path) -> None:
    ...
    text = text.replace(
        ...
        "Generated by the strict FG-DS seed-42 deployment exporter using the "
        "shared fixed-point numeric core.",
    )
...
def main() -> int:
    ...
    metadata["preprocessing_contract"] = (
        "Seed-42 feature-group-disjoint split over all 17 raw features followed "
        "by StandardScaler fit on the training partition only; validation and "
        "test are transform-only and exact raw-feature groups do not cross."
    )
    ...
    calibration["calibration_source"] = (
        "WSN-DS FG-DS training partition only after the bound train-only StandardScaler"
    )
    ...
    provenance = {
        ...
        "teacher_probability_file_sha256": sha256_file(
            context["teacher_probability_path"]
        ),
    }
    core_inventory = [
        {
    ...
    report = {
        ...
        "gates": {
            ...
            "original_code_level_macro_f1_drop": ORIGINAL_CODE_LEVEL_MACRO_F1_DROP,
            "passes_original_0_01_code_level_gate": (
                macro_f1_drop <= ORIGINAL_CODE_LEVEL_MACRO_F1_DROP
            ),
            "passes_frozen_0_03_reporting_tolerance": (
                macro_f1_drop <= MAXIMUM_MACRO_F1_DROP
            ),
        },
        ...
        "claim_boundary": (
            "Replay of already extracted 17-feature WSN-DS records from the "
            "feature-group-disjoint seed-42 test partition; no live packet capture, "
            "feature extraction, board energy, or TelosB execution. The deployment "
            "evidence establishes fixed-point execution fidelity for the preserved "
            "FG-DS seed-42 RF-KD model, not five-seed hardware replication."
        ),
    }
